...
void IPC_Helper::wakeup(pid_t pid)
{
...
unsigned long IPC_Helper::msgsnd(unsigned long id, void *ptr, size_t size)
{
	unsigned long suc;
...
ssize_t IPC_Helper::msgrcv(unsigned long id, void *ptr, size_t size)
{
	size_t suc;
	
	asm volatile( "nop" :: "d" ((uintptr_t)ptr));
	asm volatile( "nop" :: "b" (((size_t)size)|(id<<16)));
	SYSCALL(17);
	asm volatile( "nop" : "=d" (suc));
	return suc;
}
