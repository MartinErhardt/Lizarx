/*  <src-path>/src/usr/ld_a/ld.c is a source file of Lizarx an unixoid Operating System, which is licensed under GPLv3 look at <src-path>/LICENSE for more info
...
static void resolve_references(struct dynamic * to_resolve,struct dynamic * all_elfs,int elf_num)
{
	...
	if(to_resolve->jmp_rela != ST_NULL)
		for(i=0;i<to_resolve->jmp_rela_count;i++)
		{
	...
	if(to_resolve->dyn_rela != ST_NULL)
		for(i=0;i<to_resolve->dyn_rela_count;i++)
		{
			...
			if(ELF_REL_TYPE(cur_rela->info)!=ELF_REL_GLOB_DATA && ELF_REL_TYPE(cur_rela->info)!=ELF_REL_32)
				continue;
			cur_reloc_field = (st_uint_t *)(cur_rela->offset+to_resolve->runtime_addr);
			symndx=ELF_REL_BIND(cur_rela->info);
			strndx=to_resolve->sym[symndx].name;
			for(j=0;j<elf_num;j++)
			{
				...
				if((cur_sym!=ST_NULL)&&(cur_sym->section_index))
				{
					*cur_reloc_field=cur_sym->value+all_elfs[j].runtime_addr+cur_rela->addend;
					break;
				}
			}
		}
...
{
	struct dynamic mod_dynamic;
	//uprintf("hi1");
	struct elf_section_header * mod_dynamic_sh=get_dynamic(mod->header);
	st_memset(&mod_dynamic,0, sizeof(struct dynamic));
	mod_dynamic.runtime_addr=mod->runtime_addr;
	section_to_dynamic(
		        (struct elf_dyn*)((st_uintptr_t)(mod->header)+(mod_dynamic_sh->off)),
                        mod_dynamic_sh->size/sizeof(struct elf_dyn),
                        mod->header,
                        &mod_dynamic
	);
	resolve_references(&mod_dynamic, &mod_dynamic, 1);
}
void link_lib_against(struct elf_lib* first_elf,struct elf_lib * second_elf)
{
	...
	for(i=0;i<2;i++)
	{
		...
		st_memset(&dynamic_libs[i],0, sizeof(struct dynamic));
	...
	resolve_references(&dynamic_libs[0],&dynamic_libs[0],elf_num);
}
static struct elf_section_header * get_dynamic(struct elf_header* elf)
{
...
void * init_shared_lib(void* image)
{
	...
	unsigned long long size=0x0000000000;
	if (header->i_magic != ELF_MAGIC) 
	{
		uprintf("[LD.A] E: init_elf couldn't find valid ELF-Magic!\n");
		uprintf(st_itoa(((st_uintptr_t)header),16));
		return ST_NULL;
	}
#ifdef ARCH_X86
	if (header->i_class != ELF_CLASS_32)
	{
		uprintf("[LD.A] E: init_elf found elf with class != 32!\n");
		return ST_NULL;
	}
	...
	{
		uprintf("[LD.A] E: init_elf found elf with data != ELF_DATA_LITTLEENDIAN!\n");
		return ST_NULL;
	}
	if (header->type != ELF_TYPE_DYN)
	{
		uprintf("[LD.A] E: init_elf found elf with type != ELF_TYPE_EXEC\n");
	}
	if (header->i_version != ELF_VERSION_CURRENT)
	{
		uprintf("[LD.A] E: init_elf found version != ELF_VERSION_CURRENT!\n");
		return ST_NULL;
	}
	/*
	...
	*/
	...
	for (i = 0; i < header->ph_entry_count; i++, ph++)
		if(size<ph->virt_addr+ph->mem_size)
			size = ph->virt_addr+ph->mem_size;
...
{
	struct dynamic mod_dynamic;
	struct elf_symbol * ret;
	struct elf_section_header * mod_dynamic_sh=get_dynamic(mylib->header);
	st_memset(&mod_dynamic,0, sizeof(struct dynamic));
	mod_dynamic.runtime_addr=mylib->runtime_addr;
	section_to_dynamic(
		        (struct elf_dyn*)((st_uintptr_t)(mylib->header)+(mod_dynamic_sh->off)),
                        mod_dynamic_sh->size/sizeof(struct elf_dyn),
                        mylib->header,
                        &mod_dynamic
	);
	if(!(ret = get_sym_by_name(&mod_dynamic, sym_name))){
		uprintf("oops"); return 0;
	}
	return ret->value+mylib->runtime_addr;
}
static struct elf_symbol * get_sym_by_name(struct dynamic * dyn_struct, char * sym_name) 
{
	...
	do
	{
		...
		//uprintf("cmp");
		//uprintf(dyn_struct->str + sym->name);
		//uprintf("to");
		//uprintf(sym_name);
